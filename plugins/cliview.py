...
async def dailygiftt(app,query):
    ...
    if x !=None:
        ...
        ask = await app.ask(user_id, f'•︙جيد ، الان ارسل عدد المشاهدات اللي تريدها .\n\n•︙سعر كل مشاهدة = {view_price} {coin_msg}\n\n•︙اقل عدد يمكنك ارساله هو (10)',filters=filters.user(user_id))
        if ask.text:
            try:
                count = int(ask.text)
            except:
            ...
            if int(x) > int(user_info['coins']):
                ...
                return
            if count <10:
                await ask1.reply("العدد صغير جداً ، اقل عدد يمكنك ارساله هو (25)")
                return
            ses = db.get("sessions")
            y = 0
