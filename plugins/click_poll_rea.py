...
@app.on_callback_query(filters.regex("^pollrea$"))
async def dailygiftt(app,query):
    ...
    if x !=None:
        ...
        poll_price_rea = int(db.get("price_poll")) if db.exists("price_poll") else 12
        ask1 = await app.ask(user_id, f'•︙الخدمة الاساسية رشق استفتاء + تفاعلات\n\n•︙سعر كل استفتاء = {poll_price_rea}\n\n•︙اقل عدد يمكنك ارساله هو (5) \n• ارسل العدد الذي تريده',filters=filters.user(user_id))
        try:
            count = int(ask1.text)
        except:
            ...
            for i in range(count):
                ...
                for i in range(count):
                    try:
                        ...
                        oo = await reaction(ses[i], str(channel), int(post), xx)
                    except:
                        continue
                    if o:
                        y+=1
                        ...
                        continue
                for j in range(y):
                    user_info['coins'] = int(user_info['coins']) - int(poll_price_rea) 
