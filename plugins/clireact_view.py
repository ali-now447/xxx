...
@app.on_callback_query(filters.regex("^reaview$"))
async def dailygiftt(app,query):
    ...
    keys = mk(
        [
            [btn(text='رجوع', callback_data='reaview')]
        ]
    )
    ...
    if x !=None:
        ...
        reaction_price_view = int(db.get("reaction_price_view")) if db.exists("reaction_price_view") else 12
        ask = await app.ask(user_id, f'•︙جيد ، الان ارسل عدد التفاعلات اللي تريدها + مشاهدات\n\n•︙سعر كل تفاعل ومشاهده = {reaction_price_view} {coin_msg} \n\n•︙اقل عدد يمكنك ارساله هو (5)',filters=filters.user(user_id))
        if ask.text:
            try:
                count = int(ask.text)
            except:
            ...
            if ask1.text and "t.me" in ask1.text:
                ...
                try:
                    ...
                    return
            x = count * int(reaction_price_view)  / 2
            ...
            for i in range(count):
                try:
                    o = await reactionss(ses[i], str(channel), int(post), xx)
                ...
                if o:
                    y+=1
            for i in range(y):
                user_info['coins'] = int(user_info['coins']) - int(reaction_price_view) 
