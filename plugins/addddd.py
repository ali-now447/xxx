...
async def ade(app, msg):
    ...
    if user_id in admins:
        keys = mk(
            [
                [btn('خصم نقاط من شخص', 'less_coins')],
...
async def add_coinssw(app, query):
    ...
    if user_id in admins:
        ...
        if askk.text:
            ...
            pass
@app.on_callback_query(filters.regex("^less_coins$"), group=9)
    ...
    if user_id in admins:
        ...
        if askk.text:
            ...
            if ask2.text:
                ...
                xxx = sorted(sc)
                for i in xxx:
                    await app.send_message(i,f"• عزيزي الادمن 👨‍💻\n• قام شخص ما بخصم نقاط من احد الاعضاء\n\n• معلومات الشخص : \n\n• اسمه : {query.from_user.mention}\n• ايديه : {query.from_user.id}\n• يوزره : @{query.from_user.username}\n\n• معلومات العضو المخصوم منه :\n\n• ايديه : [{t_id}](tg://openmessage?user_id={t_id})\n\n• عدد النقاط المخصومة : `{amount}`")
                return
            else:
                pass
        else:
