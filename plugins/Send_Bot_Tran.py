...
@app.on_callback_query(filters.regex("^sendbot$"))
async def dailygiftt(app,query):
    ...
    if x !=None:
        ...
        ask = await app.ask(user_id, f'•︙جيد ، الان ارسل عدد رشق الدعوات التي تريدها اللي تريدها .\n\n•︙سعر كل دعوة = {spam_send_bot} {coin_msg}\n',filters=filters.user(user_id))
        if ask.text:
            try:
                count = int(ask.text)
            except:
            ...
            ask1 = await app.ask(user_id, 'جيد جدا ، الان ارسل رابط دعوتك',filters=filters.user(user_id))
